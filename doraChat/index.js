...
const queryJPWikipedia = async (search, retry, callback) => {
  ...
  const removeBracket = (s) => {
    return limitString(utils.removeBracket(s).replace(/(\s+)/g,'').replace(/（.*?）/g, '').replace(/\[.*?\]/g, ''), 50);
  }
  ...
  if (response.ok) {
    ...
    try {
      ...
      return callback(null, removeBracket(dom.window.document.querySelector("div#mw-content-text > div.mw-parser-output > p").textContent));
    } catch(err) {
    }
    try {
      return callback(null, removeBracket(dom.window.document.querySelector("li.mw-search-result > div.searchresult").textContent));
    } catch(err) {
    }
  }
...
module.exports = function(router, settings) {
  ...
  if (settings.weather) {
    ...
    router.post(`/weather`, async (req, res) => {
      ...
      let response = await fetch(`http://weather.livedoor.com/forecast/webservice/json/v1?city=${130010}`, {
        method: 'GET',
      })
