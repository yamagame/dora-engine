...
function Speech() {
  ...
  t.on("startRecording", function (params) {
    ...
    if (Object.keys(alternativeLanguageCodes).length > 0) {
      opts.config.alternativeLanguageCodes = [
        ...Object.keys(alternativeLanguageCodes),
      ];
    }
  ...
  micInputStream.on("data", function (data) {
    ...
    if (
      micInputStream.incrConsecSilenceCount() >=
      micInputStream.getNumSilenceFramesExitThresh()
    ) {
