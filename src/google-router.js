      ...
      return async function (text, source, target) {
        ...
        const request = {
          parent: translationClient.locationPath(
            config.googleTranslate.projectId,
            config.googleTranslate.location
          ),
          contents: [text],
...
const cacheDBPath =
  "synthesizeSpeech" in config && "cacheDBPath" in config.synthesizeSpeech
    ? config.synthesizeSpeech.cacheDBPath
    : null;
...
function ReqTextToSpeech(req, res, mode = "play") {
  ...
  const requestSynthesizeSpeech = (request, sndfilepath, callback) => {
    limitCacheFile(cacheDB, config.synthesizeSpeech.maxCacheSize, () => {
      ...
      } else if (request.voice.languageCode.indexOf("aws") >= 0) {
        if (!polly) {
        ...
        fs.stat(sndfilepath, (err, stats) => {
          ...
          playone(sndfilepath, (err, code) => {
            ...
            cacheDB[filename] = {
              ...
              counter: 0,
              size:
                (stats.size / 512) * 512 + (stats.size % 512 === 0 ? 0 : 512),
            };
            saveCacheDB();
            res.send("OK\n");
          });
...
router.post("/append-to-sheet", (req, res) => {
  if (
    config.googleSheet.credentialPath !== null &&
    config.googleSheet.tokenPath !== null
  ) {
